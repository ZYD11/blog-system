...
// UpdateUserStatus 更改状态
func (d UserDao) UpdateUserStatus(body req.UserBody) int64 {
	user := models.SysUser{
		UserId: body.UserId,
		Status: body.Status,
	}
	session := SqlDB.NewSession()
	session.Begin()
	update, err := session.Where("user_id = ?", user.UserId).Cols("status", "update_by", "update_time").Update(user)
	if err != nil {
		gotool.Logs.ErrorLog().Println(err)
		session.Rollback()
		return 0
